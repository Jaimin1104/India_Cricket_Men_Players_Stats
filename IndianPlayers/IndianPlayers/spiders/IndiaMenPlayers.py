import scrapy
from ..items import IndianplayersItem


class IndiamenplayersSpider(scrapy.Spider):
    name = 'IndiaMenPlayers'
    ...
    def parse_records(self, response):
        ...
        details['player_name'] = response.css('h3::text').get().capitalize() + response.css('.profile-name span::text').get().capitalize()
